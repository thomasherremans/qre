...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === "GET") {
      const r = await fetch(`${SB}/rest/v1/clients?select=id,name,ref,criteria,dossier_id&order=updated_at.desc`, { headers: sbHeaders() });
      const rows = await r.json();
      return res.status(200).json(Array.isArray(rows) ? rows : []);
    }

    if (req.method === "POST") {
      const { id, name, ref, criteria, dossier_id } = req.body || {};
      ...
      if (id) {
        ...
        if (dossier_id !== undefined) patch.dossier_id = dossier_id;
      ...
      const payload = { name: name || "", ref: ref || "", criteria: criteria || {}, dossier_id: dossier_id || null, updated_at: new Date().toISOString() };
